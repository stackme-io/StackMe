    ...
    <div className="max-w-5xl px-6 pt-5">
                    ...
                    <div className="grid sm:grid-cols-2 gap-x-8 gap-y-5">
                      {([
                        ...
                        <div key={lk}>
                          ...
                          <p className="text-sm leading-relaxed text-muted-foreground max-w-[68ch] whitespace-pre-line">{t(bk)}</p>
                        </div>
                      ))}
                    </div>
                  </div>
      ...
      {activeTab === 'manifest' && (
        <div className="max-w-2xl mx-auto">
          <h2 className="text-2xl font-medium text-foreground leading-snug mb-3 whitespace-pre-line">
            {t('heroTitle')}
          </h2>
          <p className="text-base leading-relaxed text-muted-foreground mb-2">
            {t('heroLead')}
          </p>
          <p className="text-base leading-relaxed text-foreground font-medium mb-8">
            {t('heroStatement')}
          </p>

          {([1, 2, 3, 4, 5, 6] as const).map(n => (
            <div key={n} className="py-6 border-t border-border/50 last:border-b flex gap-4">
              <span className="text-xs tabular-nums text-muted-foreground/60 flex-shrink-0 pt-1 w-6">
              ...
              <div className="flex-1 flex flex-col gap-3">
                <p className="text-sm font-semibold text-foreground">
                  {t(`block${n}Title`)}
                  {(n === 2 || n === 3) && (
                    <span className="ml-2 align-middle text-[11px] font-normal px-1.5 py-0.5 rounded border border-border text-muted-foreground/95">
                      {t(`block${n}Tag`)}
                    </span>
                  )}
                </p>
          ...
          <div className="flex gap-2.5 mt-6 mb-8 flex-wrap">
            <a href="https://github.com/stackme-io/StackMe" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 text-xs text-muted-foreground border border-border rounded-md px-3.5 py-1.5 hover:bg-muted/40 hover:text-foreground transition-colors">
              <svg viewBox="0 0 24 24" fill="currentColor" className="w-3.5 h-3.5" aria-hidden="true">
                <path d="M12 .5C5.37.5 0 5.78 0 12.29c0 5.2 3.44 9.6 8.21 11.16.6.11.82-.25.82-.56 0-.28-.01-1.02-.02-2-3.34.71-4.04-1.58-4.04-1.58-.55-1.36-1.33-1.73-1.33-1.73-1.09-.73.08-.72.08-.72 1.2.08 1.83 1.21 1.83 1.21 1.07 1.79 2.81 1.27 3.5.97.11-.76.42-1.27.76-1.56-2.67-.3-5.47-1.31-5.47-5.83 0-1.29.47-2.34 1.24-3.17-.12-.3-.54-1.52.12-3.16 0 0 1.01-.32 3.3 1.21a11.6 11.6 0 0 1 6 0c2.29-1.53 3.3-1.21 3.3-1.21.66 1.64.24 2.86.12 3.16.77.83 1.24 1.88 1.24 3.17 0 4.53-2.81 5.53-5.49 5.82.43.37.81 1.1.81 2.22 0 1.6-.01 2.9-.01 3.29 0 .31.22.68.83.56A12.02 12.02 0 0 0 24 12.29C24 5.78 18.63.5 12 .5z"/>
              </svg>
              {t('linkGithub')}
            </a>
          </div>
